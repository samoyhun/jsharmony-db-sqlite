    ...
    if(obj.columns) for(var i=0; i<obj.columns.length;i++){
      ...
      if(column.foreignkey){
        ...
        for(var tbl in column.foreignkey){
          ...
          sqlforeignkeys.push(' foreign key ('+column.name+') references '+tbl+'('+column.foreignkey[tbl]+')');
          foundkey = true;
        }
      }
    }
...
DBObjectSQL.prototype.getRowInsert = function(jsh, module, obj, row){
  ...
  function escapeVal(val){
    ...
    else if(_.isString(val)) return "'" + _this.sql.escape(val) + "'";
  ...
  for(var file_src in files){
    ...
    file_dst = DB.util.ReplaceAll(file_dst,'}}',"||'");
    sql += "update jsharmony_meta set errcode=-1,errmsg=(select '%%%copy_file:"+_this.sql.escape(file_src)+">"+file_dst+"%%%' from "+obj.name+" where rowid=(select ifnull(last_insert_rowid_override,last_insert_rowid()) from jsharmony_meta));\n";
    sql += "update jsharmony_meta set errcode=0,errmsg='';\n";
    //from ' + tbl + ' where rowid = (select ifnull(last_insert_rowid_override,last_insert_rowid()) from jsharmony_meta);
    //rslt += '%%%copy_file:'+file_src+'>'+file_dst+'%%%\n';
  }
  if(sql){
...
DBObjectSQL.prototype.resolveTrigger = function(obj, type){
  ...
  _.each(obj.triggers, function(trigger){
    if(_.includes(trigger.on,type)){
      ...
      if(trigger.exec){
        var execsql = '';
        if(!_.isArray(trigger.exec)) trigger.exec = [trigger.exec.toString()];
...
DBObjectSQL.prototype.restructureInit = function(jsh, module, obj){
  ...
  if(triggers.validate_update){
    sql += 'create trigger '+obj.name+'_before_update before insert on '+obj.name+'\n'
    sql += 'begin\n' + triggers.update + '\nend;\n';
  }
  if(triggers.insert){
  ...
  if(triggers.update){
    sql += 'create trigger '+obj.name+'_update after update on '+obj.name+'\n'
    sql += 'begin\n' + triggers.update + '\nend;\n';
  }
  if(triggers.delete){
