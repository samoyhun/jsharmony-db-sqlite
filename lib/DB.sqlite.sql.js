...
DBsql.prototype.getModelMultisel = function (ent, model, lovfield, allfields, sql_foreignkeyfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  var sql_join = 'multiparent.' + ent.map.code_val + ' = ' + tbl_alias + '.' + lovfield.name;
  ...
  sql_select_cols += ' ,coalesce(cast(' + ent.map.code_val + ' as text),cast(' + lovfield.name + ' as text)) ' + ent.map.code_val;
  sql_select_cols += ' ,coalesce(coalesce(cast(' + ent.map.code_txt + ' as text), cast(' + ent.map.code_val + ' as text)),cast(' + lovfield.name + ' as text)) ' + ent.map.code_txt;
  sql_select_cols += ', ' + ent.map.code_seq
  ...
  else if ('code' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from ' + ent.map.code + '_' + lov['code'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime'))"; }
  else if ('code_sys' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from ' + ent.map.code_sys + '_' + lov['code_sys'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime'))"; }
  else if ('code_app' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from ' + ent.map.code_app + '_' + lov['code_app'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime'))"; }
...
DBsql.prototype.putModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, encryptedfields, hashfields, enc_datalockqueries, param_datalocks) {
  ...
  //Add Keys to where
  if (keys.length >= 1){
    var sqlgetinsertkeys = 'select ' + _.map(keys, function (field) { var rslt = field.name + ' as ' + field.name; return rslt; }).join(',') + ' from ' + tbl + ' where rowid = (select ifnull(last_insert_rowid_override,last_insert_rowid()) from jsharmony_meta);';
    if(model._dbdef && (model._dbdef.table_type=='view')){
      if(keys.length == 1) sqlgetinsertkeys = 'select ' + _.map(keys, function (field) { var rslt = field.name + ' as ' + field.name; return rslt; }).join(',') + ' from ' + tbl + ' where '+keys[0].name+' = (select last_insert_rowid_override from jsharmony_meta);';
    }
...
DBsql.prototype.postModelMultisel = function (ent, model, lovfield, lovvals, foreignkeyfields, param_datalocks, datalockqueries, lov_datalockqueries) {
  ...
  if (lovvals.length > 0) {
    ...
    sql += ent.map.code_val + ' from (%%%LOVSQL%%%) multiparent where cast(' + ent.map.code_val + ' as text) in ('
    for (var i = 0; i < lovvals.length; i++) { if (i > 0) sql += ','; sql += XtoDB(ent, lovfield, '@multisel' + i); }
...
DBsql.prototype.getLOV = function (ent, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if ('code' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code + '_' + lov['code'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code2' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ent.map.code2 + '_' + lov['code2'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code_sys' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code_sys + '_' + lov['code_sys'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code2_sys' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ent.map.code2_sys + '_' + lov['code2_sys'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code_app' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code_app + '_' + lov['code_app'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('code2_app' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ent.map.code2_app + '_' + lov['code2_app'] + " where (" + ent.map.code_end_date + " is null or " + ent.map.code_end_date + ">datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
...
DBsql.prototype.getLOVFieldTxt = function (ent, model, field) {
  ...
  else if ('code' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code + '_' + lov['code'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('code2' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code2 + '_' + lov['code2'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
  else if ('code_sys' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code_sys + '_' + lov['code_sys'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('code2_sys' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code2_sys + '_' + lov['code2_sys'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
  else if ('code_app' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code_app + '_' + lov['code_app'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('code2_app' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+ ent.map.code2_app + '_' + lov['code2_app'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
