...
DBsql.prototype.putModelForm = function (ent, model, fields, keys, sql_extfields, sql_extvalues, encryptedfields, hashfields, enc_datalockqueries, param_datalocks) {
  ...
  //Add Keys to where
  if (keys.length >= 1) sql += '; select ' + _.map(keys, function (field) { var rslt = field.name + ' as ' + field.name; return rslt; }).join(',') + ' from ' + tbl + ' where rowid = (select ifnull(last_insert_rowid_override,last_insert_rowid()) from jsharmony_meta);';
