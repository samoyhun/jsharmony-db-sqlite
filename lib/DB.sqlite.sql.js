...
DBsql.prototype.getModelMultisel = function (ent, model, lovfield, allfields, sql_foreignkeyfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  sql_select_cols += ' ,coalesce(coalesce(cast(code_txt as text), cast(' + ent.map.code_val + ' as text)),cast(' + lovfield.name + ' as text)) ' + ent.map.code_txt;
  ...
  else if ('UCOD' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from ucod_' + lov['UCOD'] + " where (CODETDT is null or CODETDT>datetime('now','localtime'))"; }
  else if ('GCOD' in lov) { lovsql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ',' + ent.map.code_seq + ' from gcod_' + lov['GCOD'] + " where (CODETDT is null or CODETDT>datetime('now','localtime'))"; }
...
DBsql.prototype.getLOV = function (ent, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if ('UCOD' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'ucod_' + lov['UCOD'] + " where (CODETDT is null or CODETDT>datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('UCOD2' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'ucod2_' + lov['UCOD2'] + " where (CODETDT is null or CODETDT>datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('GCOD' in lov) { sql = 'select ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'gcod_' + lov['GCOD'] + " where (CODETDT is null or CODETDT>datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
  else if ('GCOD2' in lov) { sql = 'select ' + ent.map.code_val + '1 as ' + ent.map.code_parent + ',' + ent.map.code_val + '2 as ' + ent.map.code_val + ',' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'gcod2_' + lov['GCOD2'] + " where (CODETDT is null or CODETDT>datetime('now','localtime')) order by " + ent.map.code_seq + ',' + ent.map.code_txt; }
...
DBsql.prototype.getLOVFieldTxt = function (ent, model, field) {
  ...
  else if ('UCOD' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'ucod_' + lov['UCOD'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('UCOD2' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'ucod2_' + lov['UCOD2'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
  else if ('GCOD' in lov) { rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'gcod_' + lov['GCOD'] + ' where ' + ent.map.code_val + '=(' + valsql + ')'; }
  else if ('GCOD2' in lov) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + ent.map.code_txt + ' from '+(lov.schema?lov.schema+'_':'')+'gcod2_' + lov['GCOD2'] + ' where ' + ent.map.code_val + '1=(' + parentsql + ') and ' + ent.map.code_val + '2=(' + valsql + ')';
  }
