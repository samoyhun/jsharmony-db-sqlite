    ...
    lock(constring.database, function(release){
      ...
      var con = new sqlite3.Database(constring.database, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, function(err){
        con.run(_this.connectsql, function(err, stmt_rslt){
          if (err) { return ExecError(err, session, "DB Connect Error: "+err.toString()); }
...
function ExecError(err, callback, errprefix) {
  ...
  if (global.debug_params && global.debug_params.db_error_sql_state) DB.log((errprefix || '') + errMsg);
