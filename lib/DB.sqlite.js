...
function DBdriver() {
  ...
  this.initsql = "\
    create table if not exists jsharmony_meta as select '' context,0 errcode,'' errmsg; \
    "+_INIT_COMPLETE+";\
    ";
  this.dbpool = {};
}
...
function ExecQuery(con, sql, conComplete, callback, processor) {
  ...
  //Set start_idx and unescape \;
  for(var i=0;i<sql.length;i++){
    var stmt = sql[i];
    if(!start_idx && (stmt == _INIT_COMPLETE)){ start_idx = i; stmt = ''; }
  ...
  async.eachSeries(sql, function(stmt, stmt_cb){
    ...
    con.all(stmt, function (err, stmt_rslt) {
      ...
      else return con.all("select errcode number, errmsg message from jsharmony_meta", function(err, stmt_rslt){
        idx++;
        if(err) { }
        else if(!stmt_rslt || !stmt_rslt.length) return stmt_cb(new Error('No data returned from jsharmony_meta'));
        else if(stmt_rslt[0].number && (stmt_rslt[0].number==-1)){ notices.push(stmt_rslt[0].message); }
        else if(stmt_rslt[0].number) return stmt_cb(stmt_rslt[0]);
        return stmt_cb(err);
      });
      stmt_cb(err);
    });
...
DBdriver.prototype.Exec = function (dbtrans, context, return_type, sql, ptypes, params, callback, constring) {
  ...
  _this.ExecSession(dbtrans, constring, function (err, con, presql, conComplete) {
    ...
    //Execute sql    
    ExecQuery(con, execsql, conComplete, callback, function (rslt) {
      var dbrslt = null;
...
DBdriver.prototype.ExecTransTasks = function (dbtasks, callback, constring) {
  ...
  _this.ExecSession(null, constring, function (err, con, presql, conComplete) {
    ...
    //Begin transaction
    ExecQuery(con, presql + "begin transaction", function () { }, callback, function () {
      var transtbl = {};
      var trans = { con: con };
      ...
      async.series(dbtasks, function (dberr, rslt) {
        if (dberr != null) {
          if ((constring.jsharmony_options && (constring.jsharmony_options.stopTransactionAndCommitOnWarning)) && 
            ...
            ExecQuery(con, "commit transaction", conComplete, callback, function () {
              callback(dberr, null);
            });
          }
          else {
            //Rollback transaction
            ExecQuery(con, "rollback transaction", conComplete, callback, function () {
              callback(dberr, null);
            });
          }
        }
...
DBdriver.prototype.getContextSQL = function(context) {
  var rslt = "update jsharmony_meta set errcode=0,errmsg='',context='"+(context?this.escape(context):'')+"';";
  return rslt;
}
