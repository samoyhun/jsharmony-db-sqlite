...
function DBdriver() {
  ...
  this.initsql = "\
    create table if not exists jsharmony_meta as select 'USystem' context,0 errcode,'' errmsg,'' jsexec,null as aud_seq,0 extra_changes, null last_insert_rowid_override; \
    "+_INIT_COMPLETE+";\
    ";
  this.dbpool = {};
  this.timeout = 10000;
  ...
  if (dbtrans) {
    ...
    lock(dbconfig.database, function(release){
      if(dbconfig.database in _this.dbpool){
        var con = _this.dbpool[dbconfig.database];
        con.DeferClose({ clearTimeout: true });
        return session(null, con, _this.initsql + (dbconfig._presql || ''), onRelease(con, release));
      }
      var con = new sqlite3.Database(dbconfig.database, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, function(err){
        con.run(_this.connectsql, function(err, stmt_rslt){
          if (err) { return _this.ExecError(err, session, "DB Connect Error: "+err.toString()); }
...
DBdriver.prototype.ExecError = function(err, callback, errprefix) {
  ...
  if (this.platform.Config.debug_params.db_error_sql_state && !this.silent) this.platform.Log((errprefix || '') + errMsg);
...
DBdriver.prototype.ExecQuery = function(con, sql, conComplete, callback, processor) {
  ...
  async.eachSeries(sql, function(stmt, stmt_cb){
    ...
    if (notices.length) { return _this.ExecError(notices[0], callback, 'SQL Error: ' + last_stmt + ' :: '); }
  ...
  _this.ExecSession(null, dbconfig, function (err, con, presql, conComplete) {
    ...
    _this.ExecQuery(con, presql + "begin transaction", function () { }, callback, function () {
      ...
      execTasks(trans, function (dberr, rslt) {
        if (dberr != null) {
          if ((dbconfig.options && (dbconfig.options.stopTransactionAndCommitOnWarning)) && 
...
DBdriver.prototype.getContextSQL = function(context) {
  ...
  var rslt = "update jsharmony_meta set errcode=0,errmsg=''"+(!ignoreContext?",context='"+sqlContext+"'":"")+",jsexec='',aud_seq=null,extra_changes=0,last_insert_rowid_override=null \
                                        where (errcode<>0) or (errmsg<>'')"+(!ignoreContext?" or (context<>'"+sqlContext+"')":"")+" or (jsexec<>'') or (aud_seq is not null) or (extra_changes <> 0) or (last_insert_rowid_override is not null);";
  return rslt;
}
